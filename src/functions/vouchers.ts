 ...
 * }
*/
export const getAllVouchers = async (fastify: FastifyInstance) => {
 ...
 * }
 */
export const addVouchers = async (fastify: FastifyInstance, data: any) => {
    const connection = await fastify['mysql'].getConnection();
    ...
    try {
        ...
        const [result]: any = await connection.execute(
            `INSERT INTO vouchers (code, description, discountType, discountValue, expiresAt, usageLimit, timesUsed)
             VALUES (?, ?, ?, ?, ?, ?, ?)`,
            [code, description, discountType, discountValue, expiresAt, usageLimit, timesUsed]
        );
 ...
 */
export const updateVoucher = async (fastify: FastifyInstance, data: any) => {
    const connection = await fastify['mysql'].getConnection();
    let res = { code: 200, message: "OK." };
    ...
    try {
        ...
        const [result]: any = await connection.execute(
            `UPDATE vouchers SET 
                ...
                timesUsed = ?
             WHERE code = ?`,
            [description, discountType, discountValue, expiresAt, usageLimit, timesUsed, code]
        );
 ...
 * }
 */
export const deleteVoucher = async (fastify: FastifyInstance, id: string) => {
    const connection = await fastify['mysql'].getConnection();
    let res = { code: 200, message: "OK." };
    ...
    try {
        ...
        const usageCount = (sales as any)[0]?.count || 0;
