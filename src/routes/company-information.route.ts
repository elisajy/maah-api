...
import { getAllCompanyInfo, modifyImg, updateCompanyInfoByName } from "../functions";
...
export async function companyInfoRoute(fastify: FastifyInstance) {
    ...
    fastify.post("/update-company-info", async (request, reply) => {
        const result = await updateCompanyInfoByName(fastify, request.body);
    ...
    fastify.post("/modify-our-story-image/:key", async (request, reply) => {
        ...
        const result = await modifyImg(fastify, key, image);
        reply.code(result?.code!).send({ message: result?.message });
    });
}